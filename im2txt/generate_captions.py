...
# filename to id file
filename_to_id_file = "data/mscoco/test2014_filename_to_id.json"
# the directory of test images
image_path = '/cis/phd/cxz2081/data/mscoco/captioning/test2014/'
# Model checkpoint file or directory containing a model checkpoint file.
checkpoint_path = "model/train_bnrhn"
#checkpoint_path = "model/train/model.ckpt-906373"
# Text file containing the vocabulary.
vocab_file = "data/mscoco/word_counts.txt"
# File to save the output dict in json format.
result_file = "results/captions_test2014_bnrhns2_results.json"
...
def get_id(filename_to_id_file, image_path):
  ...
  Args:
    ...
    image_path: mscoco test image directory.

  Returns:
    filename_to_id: a dict of filename with path mapping to id.
  '''

  # load json file filename_to_idfile
  with open(filename_to_id_file, "rb") as fp:   # Unpickling
    filename_to_id = json.load(fp)

  new_dict = filename_to_id.copy()
  for key, value in filename_to_id.iteritems():
  	new_dict[image_path + key] = filename_to_id[key]
  	del new_dict[key]

  return new_dict


def get_filename(id_to_filename_file, image_path):
  ...
  Returns:
    id_to_filename: a dict of id mapping to filename with abs path.
  '''

  # load json file id_to_filename_file
  with open(id_to_filename_file, "rb") as fp:   # Unpickling
    id_to_filename = json.load(fp)

  for key, value in id_to_filename.iteritems():
  	id_to_filename[key] = image_path + value
...
def main(unused_argv):
  ...
  # id_to_filename = get_filename(id_to_filename_file, image_path)
