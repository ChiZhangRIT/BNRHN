...
import show_and_tell_model_rhn
...
def evaluate_model(sess, model, global_step, summary_writer, summary_op):
  ...
  for i in xrange(num_eval_batches):
    ...
    # ===========================================
    # # TEMP mode: to generate captions in batch.
    # output_softmax = sess.run(model.output_softmax)
    # output_softmax = np.array(output_softmax)
    # pdb.set_trace()  # output_softmax shape:(1, batch_size, caption_length)
    # ===========================================
...
def run_once(model, saver, summary_writer, summary_op):
  ...
  # model_path = 'model/train_rhn/model.ckpt-186803'
  if not model_path:
...
def run():
  ...
  with g.as_default():
    ...
    model_config = configuration.RHNModelConfig()
    model_config.input_file_pattern = FLAGS.input_file_pattern
    # model = show_and_tell_model_rhn.ShowAndTellModel_RHN(model_config, mode="testbatch")
    model = show_and_tell_model_rhn.ShowAndTellModel_RHN(model_config, mode="eval")
