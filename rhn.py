...
class Model(object):
  ...
  def __init__(self, is_training, config):
    ...
    self._noise_i = tf.placeholder(tf.float32, [batch_size, size, num_layers])
    ...
    for l in range(config.num_layers):
      with tf.variable_scope('RHN' + str(l)):
        cell = RHNCell(size, is_training, depth=depth, forget_bias=config.init_bias)
...
class RHNCell2(RNNCell):
  """Variational Recurrent Highway Layer (w/o noise)

  Reference: https://arxiv.org/abs/1607.03474
  """

  def __init__(self, num_units, depth=3, forget_bias=None):
    self._num_units = num_units
    self.depth = depth
    self.forget_bias = forget_bias
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    for i in range(self.depth):
      ...
      # coupling: c = 1 - t.
      # current_state = h * t + current_state * c
      #               = h * t + current_state * (1 - c)
      #               = h * t + current_state - current_state * t
      #               = (h - current_state)* t + current_state
      current_state = (h - current_state)* t + current_state

    return current_state, current_state


class RHNCellBoundary(RNNCell):
  """Variational Recurrent Highway Layer
  ...
  def __call__(self, inputs, state, boundary_detector, scope=None):
    current_state = tf.mul(state, boundary_detector)
    # noise_i = state[1]
    # noise_h = state[2]
    for i in range(self.depth):
      with tf.variable_scope('h_'+str(i)):
